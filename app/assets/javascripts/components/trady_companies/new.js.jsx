...
const PHONE_REGEXP = new RegExp('^[+]*[(]{0,1}[0-9]{1,3}[)]{0,1}[-\s\./0-9]*$', 'g');
  ...
  onSubmit: function(e){
  	...
  	if(!flag) {
  		...
  		const self = this;
			$.ajax({
				type: 'POST',
				url: self.props.id ? '/trady_companies/'+self.props.id : '/trady_companies',
				...
				success: function(res){
					self.openModalNotification({
						...
						content: "Add trady company is sucessfully!",
					});
				},
				error: function(err) {
					self.openModalNotification({
  ...
  render: function() {
      ...
      <form role="form" className="form-horizontal" id="new_trady_company" onSubmit={this.onSubmit} >   
       ...
       <div className="form-group">
        ...
        <button type="submit" name="commit" value="Next" className="button-primary green">
        	Next
        </button>
      </form>
