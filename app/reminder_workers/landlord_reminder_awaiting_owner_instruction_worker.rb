class LandlordReminderAwaitingOwnerInstructionWorker
  ...
  def perform
    ...
    #get the count and then send them an email for each agency admin on the system
     ...
     maintenance_requests = MaintenanceRequest.all.includes(property:[:landlord]).joins(:action_status).where(:action_statuses => { :agent_status =>"Awaiting Owner Instruction" }).distinct
