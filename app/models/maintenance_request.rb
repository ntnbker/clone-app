...
class MaintenanceRequest < ApplicationRecord
  ...
  def self.find_maintenance_requests(current_user, params)
    ...
    if current_user.logged_in_as("AgencyAdmin")
      maintenance_request_array = MaintenanceRequest.where({ agency_admin_id: current_user.get_role("AgencyAdmin").roleable_id}).joins(:action_status).where(:action_statuses => { :agent_status => params}).includes(:images).distinct
    elsif current_user.logged_in_as("Agent")
      maintenance_request_array = MaintenanceRequest.where({ agent_id: current_user.get_role("Agent").roleable_id}).joins(:action_status).where(:action_statuses => { :agent_status => params}).includes(:images).distinct
