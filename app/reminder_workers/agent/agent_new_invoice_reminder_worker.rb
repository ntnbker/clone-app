class Agent::AgentNewInvoiceReminderWorker
  ...
  def perform
    ...
    agent.each do |agent|
        ...
        maintenance_requests = MaintenanceRequest.all.where({ agent_id: agent.id}).joins(:action_status).where(:action_statuses => { :agent_status =>"New Invoice" }).distinct
        maintenance_requests.each do |maintenance_request|
          AgentMailer.agent_new_invoice_reminder(maintenance_request).deliver
        end 
    end 
