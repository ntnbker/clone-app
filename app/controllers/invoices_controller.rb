class InvoicesController < ApplicationController
  ...
  def mark_as_paid
    ...
    #if the amount of invoices that have been submitted and are not paid is 1 or more  dont say job complete



    #THIS IS WHERE WE FIGURE OUT IF THE MAINTENACE REQUEST HAS BEEN CLOSE COMPLETELY OR NOT.
    if  maintenance_request.invoices.where(paid:false, delivery_status:true).count >= 1 || maintenance_request.uploaded_invoices.where(paid:false, delivery_status:true).count >= 1
      #do nothing 
      ...
      log = Log.create(maintenance_request_id:maintenance_request.id, action:"Maintenance completed.")
  ...
  def void_invoice
    ...
    if !params[:message].empty?
      ...
      if current_user.logged_in_as("Trady")
        ...
        TradyInvoiceVoidEmailWorker.perform_async(maintenance_request.id, invoice.id) 
        person = "Tradie"
