class AgencyAdmin::AgencyAdminCancelledWorkOrderReminderWorker
  ...
  def perform
    ...
    agency_admins.each do |agency_admin|
        ...
        maintenance_requests = MaintenanceRequest.all.where({ agency_admin_id: agency_admin.id}).joins(:action_status).where(:action_statuses => { :agent_status =>"Cancelled Work Order" }).distinct
        maintenance_requests.each do |maintenance_request|
        
          AgentMailer.agency_admin_cancelled_work_order_reminder(maintenance_request).deliver
        end 
    end 
