class Agent::AgentOutstandingMaintenanceRequestReminderWorker
  ...
  def perform
    ...
    agent.each do |agent|
        ...
        send_work_order_count = MaintenanceRequest.all.where({ agent_id: agent.id}).joins(:action_status).where(:action_statuses => { :agent_status =>"Send Work Order" }).distinct.count
        ...
        AgentMailer.agent_outstanding_maintenance_requests(agent, new_count, quote_requested_count, quote_recieved_count, new_invoice_count, cancelled_work_order_count, deferred_maintenance_request_count, send_work_order_count).deliver
