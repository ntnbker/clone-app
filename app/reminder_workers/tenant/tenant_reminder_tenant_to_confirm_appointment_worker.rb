...
class TenantReminderTenantToConfirmAppointmentWorker
  ...
  def perform
    ...
    #get the count and then send them an email for each agency admin on the system
     ...
     maintenance_requests = MaintenanceRequest.all.includes(:tenants,:trady,:quotes,property:[:landlord]).joins(:action_status).where(:action_statuses => { :agent_status =>"Tenant To Confirm Appointment" }).distinct
     ...
     maintenance_requests.each do |maintenance_request|
      ...
      if maintenance_request.trady
        ...
        TenantMailer.reminder_tenant_to_confirm_appointment(maintenance_request, trady, property,tenant).deliver
      end 
